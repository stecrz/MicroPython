...
def setup_motion_switch(spi, cs, freq=25, thresh_act=200, samp_act=200, thresh_inact=120, samp_inact=50, mrange=2):
    ...
    write_reg(0x27, 0b00111111)  # loop-mode motion detection, enable act./inact. reference
    write_reg(0x2B, 0b01000000)  # map AWAKE bit to INT2 (gate of the switch), active=high (MSB)
    write_reg(0x2D, 0b00000110)  # measurement mode, autosleep mode (instead of wakeup mode)
    # alt.: write_reg(0x2D, 0b00001010)# measurement mode, wakeup mode


def deepsleep():  # TODO replace by >71m sleep
    import machine
...
# setup(SPI(1, polarity=0, phase=0), Pin(15, Pin.OUT))
...
#         display.println("was %d for %.1f s" % (v, round(time.ticks_diff(tmr_new, tmr) / 1000, 1)))
#         tmr = tmr_new
#         v = v_new
