...
class PulseVisualizer:
    # """ displays the pulse, which is read from the ADC (sensor must be connected) on an oled display. """
    ...
    def _append_value(self, y, dx=5):
        # """ appends the given y-value to the pulse graph on the oled display by connecting the
    ...
    def update(self, adc_value, bpm):
        # """ updates the display with the given value from the ADC (must be in range 0-1024!)"""
        # graph:
...
sensor = PulseSensor()
display = PulseVisualizer(128, 64)
led = machine.Pin(16, machine.Pin.OUT)

while True:
    sensor.update()
    display.update(sensor.value, sensor.bpm)
